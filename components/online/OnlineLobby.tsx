...
type Props={onBack:()=>void;onGameStart:(socket:Socket,roomId:string,color:"w"|"b",startAugmentTiers?:AugmentTier[])=>void};
...
export default function OnlineLobby({onBack,onGameStart}:Props){
 ...
 useEffect(()=>{const s=io(URL,{transports:["polling","websocket"],reconnection:true});setSocket(s);s.on("connect",()=>setStatus("CONNECTED"));s.on("connect_error",()=>setStatus("SERVER OFFLINE"));s.on("room-created",({roomId})=>{setCreated(roomId);setStatus("WAITING FOR OPPONENT");setBusy(false)});s.on("room-joined",({roomId,color,startAugmentTiers})=>{startedRef.current=true;setBusy(false);onGameStart(s,roomId,color,startAugmentTiers)});s.on("game-start",({roomId,color,startAugmentTiers})=>{startedRef.current=true;onGameStart(s,roomId,color,startAugmentTiers)});return()=>{if(!startedRef.current)s.disconnect()}},[onGameStart]);
